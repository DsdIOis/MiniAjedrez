...
public abstract class PiezaAjedrez extends Pieza {
	...
	public boolean movimientoDentroLimites (int toFila, int toColumna) {
		...
		if(!(toFila==getFila()&&toColumna==getColumna())&&(toFila>=0&&toFila<=7&&toColumna>=0&&toColumna<=7)) {
				return true;
		}
		else
			return false;
...
//	métodos auxiliares
	/**
	 * Este método recorre el tablero desde la posición inicial de la pieza hasta llegar a un extremo del 
	...
	protected void casillasVisitables (ArrayList<Casilla> resultado,int incFila, int incColumna){
		int f=getFila()+incFila;
		int c=getColumna()+incColumna;
		while(queHay(f, c)== null && movimientoDentroLimites(f, c)) {
			resultado.add(resultado.size(), new Casilla(f, c));
			f=f+incFila;
			c=c+incColumna;
		}
		if(queHay(f, c)!= null&& queHay(f, c).getJugador().getId() != this.getJugador().getId())
			resultado.add(resultado.size(), new Casilla(f, c));
		}
